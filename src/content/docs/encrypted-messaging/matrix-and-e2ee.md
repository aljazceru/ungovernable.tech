...
Matrix is a federated open standard for real-time communication: chat, VoIP, IoT, file sharing. The unit of identity is `@user:homeserver.tld`; rooms span multiple homeservers via federation. Compared to Signal, it is more complex, more flexible, and leaks more metadata. Compared to Nostr, it has stronger consistency and weaker identity portability.
...
For rooms, a sender publishes a *session* (a ratcheting symmetric chain) to all members. Members encrypt incoming Megolm sessions per-device using Olm. Properties:
...
### Cross-signing and key verification

Identity verification across devices via cross-signed keys. UX is emoji or numeric short-code comparison. Reduces the TOFU footgun substantially since 2020.

### MLS migration

Matrix has an MLS profile in development that replaces Olm/Megolm with the IETF MLS standard (RFC 9420). Production rollout is staged; stable group E2EE with TreeKEM for large rooms.
...
The room is owned by its origin server and replicated to all servers with members. Server outages mean partial outages for that server's users; the room continues elsewhere.
...
- Disable URL previews (the homeserver fetches URLs, leaking room contents).
- Disable presence unless needed.
- Use ephemeral or disposable MXIDs for sensitive joins.
- Pair E2EE with Tor SOCKS for the client.
- Audit room state for unwanted user-impersonating bots.

### Account recovery

Cross-signing plus key backup means recovery is possible from a Recovery Key (essentially a passphrase-encrypted backup of decryption keys). Lose it AND all devices, and you cannot decrypt history.
...
- Inconsistent client crypto correctness. Bugs surface at the boundary of Olm/Megolm and the room state machine.
...
- Sliding Sync (MSC3575): dramatically faster client cold-start; deployed.
