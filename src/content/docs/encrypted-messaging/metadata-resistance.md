...
- **Multi-server** by default — split queues across operators.
...
- **Mailbox-based delivery** — recipient pulls; sender pushes to swarm of nodes hosting the mailbox.
...
- **Designed for journalists / activists** in repressive regimes.
...
- **Your contacts in non-adversarial settings.** Real-world graph (the people you actually talk to in person) leaks through phone records, photos, posts.
...
For ultra-high-stakes (whistleblowing, journalism in repressive regimes), the recommended stack is **Tails OS** (amnesic) + **Tor** + (Cwtch or SecureDrop for source-journalist contact, depending on async needs) + an air-gapped device for handling content.
...
- **Battery cost** of always-on Tor / mixnet routing.
