...
Most modern E2EE messengers (WhatsApp, Google Messages RCS, Skype Private Conversations, Wire 1.0, Element early days) are direct or close ports of Signal Protocol. The combination of:
...
`SK` is the root key for the Double Ratchet. Alice attaches her ephemeral public key + identity key to her first message; Bob can re-derive `SK`.
...
This means: even if a future quantum adversary records today's traffic and breaks all ECDH, they still face an unbroken ML-KEM. Currently deployed in Signal mobile clients since fall 2023.
...
- **Endpoint compromise.** Malware on phone reads everything pre-encryption.

These motivate the federated/decentralized alternatives: [Matrix and E2EE](/encrypted-messaging/matrix-and-e2ee), [Nostr Deep Dive](/encrypted-messaging/nostr-deep-dive), SimpleX.
...
- **Backups outside protocol** — undermines deletability if used.
