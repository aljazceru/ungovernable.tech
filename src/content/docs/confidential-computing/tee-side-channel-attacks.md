...
- **Foreshadow / L1TF (2018).** Read enclave memory via L1 cache side channel. Required microcode + OS mitigations; effectively broke SGX's confidentiality guarantee on affected CPUs until patched.
- **Plundervolt (2019).** Undervolting via `MSR 0x150` induced computation faults inside enclaves, leaking RSA/AES keys. Fixed by Intel disabling software voltage control on SGX-capable CPUs.
- **LVI (Load Value Injection, 2020).** Inverse of Spectre; attacker injects values into victim enclave loads. Mitigations are expensive (LFENCE everywhere) — some real deployments accept the risk rather than take the perf hit.
...
- **Ongoing:** cache-timing, branch-predictor-based key extraction against crypto libraries inside enclaves remains a research area. Use constant-time crypto (libsodium, ring) and disable SMT (HyperThreading) in production.

### AMD SEV / SEV-ES / SEV-SNP

- **SEVurity, CacheWarp, SEVered (2018–2023).** Pre-SNP SEV had integrity-free memory encryption → malicious hypervisor could remap or replay ciphertext. SNP's reverse-map table (RMP) closed these.
- **undeSErVed / AMDSEV-ENCRYPTED (2021–).** Side-channel leaks via the NPT (nested page tables) the hypervisor still controls; partially mitigated by SNP.
- **BadRAM (2024).** Physical attack: rogue SPD on DIMMs fools the CPU about RAM size and bypasses SNP memory protection. Mitigation: SPD verification, trusted DIMM provisioning. Not relevant against purely-remote attackers.
- **CacheWarp (2023).** Architectural flaw allowing rollback of writes in SEV-ES/SNP VMs; AMD issued microcode fix.
- **TCB freshness is critical.** Most SEV CVEs are fully mitigated by current microcode + PSP firmware — but only if deployed.
...
- **GSP firmware CVEs (2023–).** Several DoS and information-disclosure issues; patched via GPU firmware updates. Small surface area relative to CPU TEEs but not zero.
- **Timing / thermal side channels** across CUDA kernels inside a CC GPU remain underexplored publicly.
- **PCIe-side attacks** largely blocked by SPDM encryption, but implementation bugs in the guest NVIDIA driver could still leak.

### Apple Secure Enclave / PCC

- **checkm8 (2019)** — bootROM exploit on older iPhones; not applicable to PCC but illustrates that "immutable ROM" isn't.
- **PCC itself has no publicly disclosed breaks as of early 2026**, but the transparency log + closed-source server silicon mean any audit is indirect.
...
1. **Microcode** — required, deploy ASAP after each advisory.
2. **Compiler / kernel** — retpolines, `LFENCE`, speculative-load hardening (`SLH`), disabling SMT.
3. **Architectural** — newer CPU gens fix some at silicon level.

Operational posture: **TCB minimum version enforcement in attestation** is the only way to be sure clients only talk to patched silicon. Don't rely on "we patched the fleet" — an attacker can always try to route you to an unpatched node.
...
Mitigations:
- **Oblivious algorithms** (ORAM, oblivious RAM) — expensive.
- **Data-independent access patterns** in crypto code.
- **Large-page / batched access** to reduce fault observability (partial).
- **Move to TDX or SEV-SNP** — VM-level TEEs don't expose per-page faults to the hypervisor in the same way, though coarser-grained signals remain.

---

## Supply-Chain / Platform Root Attacks

Most catastrophic. A single compromised attestation signing key can mint quotes for enclaves that don't exist.
...
- **Cloud provider attestation collateral tampering** — mitigated by pinning vendor roots, not cloud-operator roots. Don't trust MAA / NRAS blindly; cross-check against Intel/AMD/NVIDIA-published collateral.

---

## Covert and Leakage Channels Specific to Inference

- **Hertzbleed (2022) / frequency-throttling leaks.** Dynamic frequency scaling depends on data operands → remotely observable timing leaks even inside a TEE. Mitigation: disable turbo on high-assurance hosts, or use constant-time/operand-independent code paths for crypto. Relevant for any AES/RSA inside an enclave.
...
- *CacheWarp* — Schwarzl et al., USENIX Security 2024
