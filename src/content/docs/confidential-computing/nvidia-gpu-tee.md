...
tags:
  ...
  - "confidential-ai"
---
*The only production GPU confidential-computing stack as of 2026. Every "confidential LLM inference on frontier models" deployment is built on it.*

---

## Why a GPU TEE matters

CPU TEEs (SGX, TDX, SEV-SNP) solved "data in use" for CPU workloads. Modern inference and training are GPU-bound, though, so if the prompt leaves the CPU enclave in plaintext across the PCIe bus to a non-confidential GPU, the whole chain breaks. A malicious hypervisor, a PCIe-snooping cloud operator, or a rogue firmware blob on the GPU can read every token.

NVIDIA Confidential Computing first shipped on H100 / Hopper in 2023, expanded on H200 in 2024, and on Blackwell B100/B200/GB200 in 2024-2025. It extends the TEE boundary across the PCIe link and into the GPU itself.
...
3. CC-On (DevTools): looser mode for debugging. Do not ship production with this.
4. Multi-GPU / NVLink CC (Blackwell): NVLink traffic between GPUs in the same confidential domain is also encrypted and attested. This enables multi-GPU models (tensor or pipeline parallel) inside a single TEE trust domain. Hopper's NVLink is not encrypted, so multi-GPU on H100 CC forces PCIe routing with a heavy perf cost. This is a major Blackwell upgrade.

---

## The attestation flow (end to end)

1. The client opens TLS to the relying service and requests attestation evidence before sending the prompt.
2. The CPU TEE (TDX/SEV-SNP) generates its quote with a nonce supplied by the client, including the hash of the GPU attestation token it just received.
3. The GPU produces its own quote (via GSP) over the same nonce plus the current firmware version and CC mode status, signed by the device-unique attestation key.
4. The client (or a verifier the client trusts) checks:
   - The CPU quote against Intel PCS / AMD KDS collateral.
   - The GPU quote against NVIDIA's device-certificate chain plus the NRAS revocation list.
   - Firmware version is at least the policy-defined minimum.
   - The measurement (MRTD for TDX; measurement of OVMF + kernel + initrd + rootfs) matches the expected value for the published image.
5. The client derives a session key bound to both quotes. From this point the prompt is encrypted to the dual-attested TEE and no component in the cloud can decrypt it.
...
Rough numbers (H100 CC, 70B-class dense LLM inference, published and independently measured in 2024-2025):
...
Rule of thumb: for steady-state LLM serving, assume around 10% cost. For training or workloads that thrash PCIe, the cost is higher and may be prohibitive until Blackwell's encrypted NVLink is the norm.
...
- Model extraction via the API. The TEE prevents raw weight theft; it does not prevent slow distillation via queries.
- Driver compromise on the host. The host NVIDIA driver is untrusted by design; that is the point. But a buggy guest driver inside your CVM can still leak. Keep the guest-side `nvidia-open` driver pinned and audited.
...
- Covert channels via power or thermal. Co-located TEE workloads can sometimes be distinguished by power draw observable to the cloud operator. Not a confidentiality break for plaintext, but a weak fingerprint channel.
...
| **GCP** | A3 Confidential | Intel TDX (Sapphire Rapids / EMR) | H100 CC | Preview as of late 2025 |
...
2. Pin GSP firmware at or above the minimum version that patches the latest advisory (track `nvidia.com/en-us/security/`).
3. Use the NVIDIA Confidential Computing SDK / NVTrust for attestation collection. Do not roll your own quote parser.
