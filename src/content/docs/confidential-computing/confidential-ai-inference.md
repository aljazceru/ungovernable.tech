...
tags:
  ...
  - "privacy"
---
*Running ML inference where the model owner does not see the inputs and the user does not see the weights, with both sides able to cryptographically verify the promise.*
...
1. User to provider. Users hand raw prompts, images, documents, medical records, or code to a remote GPU they do not control. The provider (and anyone with subpoena or coercion power over it) sees everything.
2. Provider to user. Providers must ship model weights to any host that runs the workload. Weights get stolen, leaked, or exfiltrated by customers, insiders, or compromised hypervisors.
3. Both to infrastructure. Cloud operators, hypervisors, orchestration layers, and supply chains sit between user and model and can silently swap models, log prompts, or tamper with outputs.
...
A fully confidential inference service satisfies all of the following for every request:
...
No production system hits 7/7 yet. The state of the art (Apple PCC, NVIDIA H100 CC plus attestation, Tinfoil, Phala, Oasis, Marlin) hits 4-6 depending on threat model.
...
The whole model runs inside an Intel TDX or AMD SEV-SNP confidential VM. No GPU.

- Fits: models below 20B parameters that can tolerate CPU latency, or fine-tuned small transformers (embeddings, classifiers, tool-call routers).
- Providers: Azure DCesv5 (TDX) or DCasv5 (SEV-SNP), GCP C3 (TDX) or N2D (SEV-SNP), AWS M6a/C6a (SEV-SNP).
- Example stacks: Gramine + llama.cpp, Enarx, Occlum, MSFT Confidential Inferencing Containers.
- Limitation: no matrix-multiply accelerator. 50-200x slower than a GPU for large models.

### 2. CPU-TEE plus GPU-TEE (the modern pattern)

A confidential VM (TDX/SEV-SNP) is paired with a confidential GPU (NVIDIA H100 / H200 / Blackwell in CC mode). All PCIe traffic between them is AES-GCM encrypted; both endpoints attest jointly.
...
- Caveat: 5-15% throughput cost; first-token latency goes up by 30-120ms from the attestation handshake.
...
- FHE (Zama Concrete-ML, OpenFHE): input privacy and weight privacy, but ~10³-10⁶× overhead. Only viable for tiny models today.
- MPC (CrypTen, MP-SPDZ): 2-3 non-colluding parties, interactive. Good for specific enterprise joint-compute deals.
- ZKML (EZKL, Giza, RISC Zero + onnx): proves the output was computed correctly from a committed model. Not input privacy by itself. Pair with TEE for the full story.

### 5. On-device inference

Run the model on the user's own hardware: Apple Neural Engine, Qualcomm Hexagon, local llama.cpp on M-series or 5090. No confidentiality problem because the user is the trusted party.

- Gold standard for privacy when the model fits. Always ask "can this run locally?" before reaching for a TEE.
...
### Apple Private Cloud Compute (PCC), 2024 onward

The most ambitious consumer deployment. An iPhone can offload an Apple-Intelligence request to PCC servers where:

- Custom Apple Silicon server nodes run a hardened variant of iOS/Darwin with Secure Enclave, Secure Boot, and sealed memory.
- Every build image is reproducibly hashed and published to a transparency log; clients refuse to send prompts to any node whose image hash is not in the log.
...
Why it matters: the first mass-market system where "the server operator genuinely cannot see your prompt" is a deployed guarantee rather than marketing. Threat model writeup: `security.apple.com/blog/private-cloud-compute`.

What it is not: open. Only Apple builds on Apple silicon can join. No third-party audit of running instances; you trust the transparency log and Apple's signing keys.
...
- CPU-TEE (SEV-SNP / TDX) plus NVIDIA CC GPU hosting of open-weights LLMs (Llama 3.x, DeepSeek, Mistral).
...
- Decentralized TEE marketplace (Intel SGX, then TDX, with NVIDIA CC support).
...
- The `dstack` SDK packages docker-compose workloads into TDX CVMs with deterministic attestation.
...
- Enterprise Kubernetes plus CoCo stack; attestation-gated secret injection into confidential pods.
...
- Anjuna: "lift and shift" confidential VM wrapper. No code changes; auto-enables SEV-SNP/TDX plus attestation gating on AWS/Azure/GCP.

---

## Attack surface specific to inference

Even a perfect TEE does not give you a perfect confidential-inference system. Model-specific and workflow-specific leaks:

### Prompt and activation leakage

- Timing side channels: decode latency varies with token choice, KV-cache state, and speculative decoding hit/miss. An observer of response timing alone can sometimes reconstruct which tokens were generated. Mitigations: constant-time decoding (rare), padded batches, fixed `max_tokens`, jittered emission.
- Memory access patterns: even inside an enclave, DRAM traffic is visible to the memory controller. Sparse-MoE routing has been shown to leak the active expert, which leaks the token class. Dense models are safer; MoE in TEE is an open problem.
- Speculative decoding across tenants: shared draft models leak cross-request information. Do not share drafts across confidentiality domains.
...
- Logit leakage via streaming: per-token streaming plus timing equals more side-channel surface than batched completion. Prefer batched for high-sensitivity use.
...
- Page-swap and cold-boot: older SGX could be coerced into paging EPC; SEV-SNP and TDX mitigate but do not eliminate this. Disable hibernation on confidential hosts.
...
- Collateral DoS: Intel and AMD attestation services can be unavailable. Cache TCB info and verifier collateral locally; support offline verification.
...
- Build reproducibility: if you cannot rebuild the enclave image bit-for-bit, your attestation quote means "trust the build server," not "trust the code." Use SLSA L3+, sigstore, and transparency-logged image hashes.
...
2. Where does the plaintext prompt first appear? Client TLS to TEE public key? Verify the TLS endpoint is inside the enclave, not a front-end proxy.
3. How is the model key released? KMS to enclave via attested channel? Or baked into the image (rotation nightmare)?
...
7. What does the user verify? A one-line "this is secure" UI is theater. Give them the image hash plus a way to check it against the transparency log, even if 99% ignore it.
